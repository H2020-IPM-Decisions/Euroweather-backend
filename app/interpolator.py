...
class Interpolator():
    ...
    def populate(self, llen, nlen):
        ret = []
        for _ in range(0, llen):
            line = []
            for _ in range(0, nlen):
                line.append({})
            ret.append(line)
        return (ret)
    ...
    def setWindSpeed(self):
        ...
        for ii in range(0, olen):
            if (np.any(ff10m[ii] is None) and np.any(u10m[ii] is not None) and np.any(v10m[ii] is not None)):
                line = []
    ...
    def setHumidity(self):
        ...
        for ii in range(0, olen):  # time
            if (rh[ii] is None):
                if (q[ii] is not None and t[ii] is not None and ps[ii] is not None):
                    line = []
                    ...
                    rh[ii] = line
                elif (td[ii] is not None and t[ii] is not None):
                    line = []
                    # we have td,t => rh
                    ...
                    rh[ii] = line
            if (td[ii] is None):
                if (rh[ii] is not None and t[ii] is not None):
                    line = []
                    # we have rh and t => td
                    ...
                    td[ii] = line
            if (q[ii] is None):
                if (td[ii] is not None and ps[ii] is not None):
                    line = []
                    # we have td and ps => q
    ...
    def satVapPres(self, t, ice=False):  # t, ice
        ...
        if(ice):
            ...
            log10ei = -9.09718*(273.16/t-1)-3.56654*math.log10(273.16/t) +\
                       0.876793*(1-t/273.16) + math.log10(6.1071)
            ei = 10**log10ei
            return ei
        else:
    ...
    # data is stored: ret=[<latlon1>,<latlon2>...<latlonn>],
    #                      <latlon>=[<time1>...], <time>={var1:0,var2:0...}
            ...
            try:
                ...
                if (times[0] != self.undef):
                    ...
                    if (lead in self.minutesOfTheHour):
                        ...
                        # ...for instance:
                        # ...getData(['air_temperature_2m','t2m','temperature_at_2_meter'],ii)
                        # ...the first variable with any data will then be used
                        # ...if no data is available for any of the alternative variables, "None"
                        # ...will be returned.
                        ...
                        self.tp.append(self.getData(['total_precipitation'], ii))
