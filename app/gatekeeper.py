...
class Gatekeeper():
    ...
    def collect(self):
        ...
        for filename in os.listdir(self.directory):
            ...
            if (filename.endswith(".res") or filename.endswith(".tmp")):
                if self.file_age(path) > 600:
                    os.remove(path)
            else:
                pass
...
file_pattern = "{%s080100..%s}" % (datetime.now().year-2,
...
if len(sys.argv) == 2:
    ...
    try:
        ...
        exit(1)
    file_pattern = "{%s..%s}" % (requested_time,
                                 datetime.strftime(datetime.now(), "%Y%m%d%H"))
